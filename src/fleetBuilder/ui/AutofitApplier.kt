...
import fleetBuilder.autofit.MLPlayerAutofitDelegate
...
object AutofitApplier {
    ...
    fun applyVariantInRefitScreen(
        ...
        ReflectionUtils.invoke(shipDisplay, "setSuppressMessages", true)
        ...
        try {
            ...
            if(!Global.getSettings().isInCampaignState) {
                copyVariant(baseVariant, loadout, dontForceClearDMods, dontForceClearSMods)
            } else {
            ...
            showError("ERROR: Failed to apply ship variant", e)
            //e.printStackTrace()
        }

        ReflectionUtils.invoke(shipDisplay, "setSuppressMessages", false)
    }
    ...
    fun copyVariant(to: ShipVariantAPI, from: ShipVariantAPI, dontForceClearDMods: Boolean = false, dontForceClearSMods: Boolean = false) {
        ...
        to.hullMods.toList().forEach { mod ->
            ...
            if (dontForceClearSMods && to.sMods.contains(mod))
                return@forEach
            if (dontForceClearDMods && getAllDMods().contains(mod))
                return@forEach
