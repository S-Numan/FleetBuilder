                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    if(Global.getSettings().fileExistsInCommon("$dirPath$prefix/$shipPath")){
                        ...
                        try {
                            ...
                            Global.getLogger(this.javaClass).error("Failed to read ship variant JSON at /saves/common/$dirPath$prefix/$shipPath\n", e)
                            continue
                        }
                        ...
                        Global.getLogger(this.javaClass).warn("shipPath in path directory /saves/common/$dirPath$prefix linked to ship variant at /saves/common/$dirPath$prefix/$shipPath\nHowever, no file was found at that location")
    ...
    fun getAllAutofitSpecsForShip(hullSpec: ShipHullSpecAPI): List<AutofitSpec> {
        ...
        for(variant in variants) {
            if(variant.isGoalVariant && ModSettings.showCoreGoalVariants)
                variantSpecs.add(AutofitSpec(variant.hullVariantId, variant.displayName, "Core Autofit Variant", variant.hullSpec.spriteName))
            else if(!variant.isGoalVariant && ModSettings.showCoreNonGoalVariants)
                variantSpecs.add(AutofitSpec(variant.hullVariantId, variant.displayName, "Core Variant", variant.hullSpec.spriteName))
        }
        ...
        for ((i, variant) in loadouts.withIndex()) {
            variantSpecs.add(AutofitSpec(variant.hullVariantId, variant.displayName, source[i].getDescription(), variant.hullSpec.spriteName, missings[i]))
        }
        ...
        for(ship in ships) {
            for (dir in shipDirectories) {
                ...
                if(!dir.containsShip(ship.hullVariantId)) {
                    Global.getLogger(this.javaClass).error("Ship of variant id ${ship.hullVariantId} has a prefix of ${dir.prefix} but was not in the ship directory of that prefix.")
